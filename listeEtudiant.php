  ...
  if(isset($_FILES['fichier']) and $_FILES['fichier']['error']==0)
{
 ...
 if (!in_array($extension_upload, $extensions_autorisees))
 {
 exit("Erreur, Veuillez inserer une version pdf svp (extensions autorisées: .pdf )");
 }
 $nom_fichier=$Apog."(2).".$extension_upload;
 if(!move_uploaded_file($temp_name,$dossier.$nom_fichier))
 {
 exit("Problem dans le telechargement de la version, Ressayez");
 }
 $f2=$nom_fichier;
...
{  
  ...
  if(isset($_FILES['fichier']) and $_FILES['fichier']['error']==0)
  {
     ...
     $extension_upload = strtolower($extension_upload);
     $extensions_autorisees = array('pptx');
     if (!in_array($extension_upload, $extensions_autorisees))
     {
     exit("Erreur, Veuillez inserer une presentation svp (extensions autorisées: .pptx)");
     }
     $nom_fichier=$Apog."(3).".$extension_upload;
     if(!move_uploaded_file($temp_name,$dossier.$nom_fichier))
     {
     exit("Probleme dans le telechargement de la presentation, Ressayez");
     }
     $f3=$nom_fichier;
     $q = "SELECT * FROM etudiant WHERE Apogee=$Apog";
    $r=mysqli_query($link,$q);
  ...
  if(isset($_FILES['fichier']) and $_FILES['fichier']['error']==0)
{
 ...
 if (!in_array($extension_upload, $extensions_autorisees))
 {
 exit("Erreur, Veuillez inserer une attestation svp (extensions autorisées: .pdf)");
 }
 $nom_fichier=$Apog."(4).".$extension_upload;
 if(!move_uploaded_file($temp_name,$dossier.$nom_fichier))
 {
 exit("Problem dans le telechargement de l'attestation, Ressayez");
 }
 $f4=$nom_fichier;
 $q = "SELECT * FROM etudiant WHERE Apogee=$Apog";
$r=mysqli_query($link,$q);
  ...
  if(isset($_FILES['fichier']) and $_FILES['fichier']['error']==0)
{
 ...
 if (!in_array($extension_upload, $extensions_autorisees))
 {
 exit("Erreur, Veuillez inserer une fiche d'evaluation svp (extensions autorisées: .pdf )");
 }
 $nom_fichier=$Apog."(5).".$extension_upload;
 if(!move_uploaded_file($temp_name,$dossier.$nom_fichier))
 {
 exit("Problem dans le telechargement de la fiche d'evaluation, Ressayez");
 }
 $f5=$nom_fichier;
 $q = "SELECT * FROM etudiant WHERE Apogee=$Apog";
$r=mysqli_query($link,$q);
...
<body>
   ...
   $id_stage = $_SESSION['id_stage'];
    if($id_stage==NULL)
    {
      ...
      echo "<br/><a href='./stageInfo.php'>Ajouter un Stage</a>";
    }
    else { 
   ?>
   <h3>Entreprise</h3>
...
<h2>Déposer</h2>
<label for="fichier">Version Finale du rapport :</label>
<?php  
